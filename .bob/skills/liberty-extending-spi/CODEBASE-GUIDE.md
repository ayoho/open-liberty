...
### 2.1 Product Extension Mechanism

A product extension is a directory tree structured like a Liberty install root. It is registered by placing a `.properties` file in `${wlp.install.dir}/etc/extensions/<extension-name>.properties` containing:
...
### 2.2 Declarative Services as the Wiring Model

**What it is**: DS (OSGi Compendium spec 112) is the universal component model inside Liberty. Every Liberty service — authentication, web container, JDBC pool, application manager — is a DS component. The key properties that make DS the right model for Liberty extensions:

- **Lazy activation**: DS components are not instantiated until a consumer binds to them. A product extension's components will never load if the feature is installed but no other component references them.
- **Dynamic reference binding**: DS watches the OSGi service registry for services matching `@Reference` declarations and calls bind/unbind methods as services appear/disappear. This enables extensions to plug into the Liberty pipeline without modifying any Liberty code.
- **Config injection**: DS delivers `Map<String,Object>` from Config Admin to `@Activate` and `@Modified`. This means extension components receive `<myConfig>` element values from `server.xml` automatically, using the same mechanism as all Liberty components.
- **Service lifecycle safety**: When a required dependency disappears (e.g., a keystore is removed), DS deactivates all components that depended on it automatically. No null-check boilerplate in extension code.

**The `@Modified` contract is critical**: If a DS component does not declare a `@Modified` method, DS deactivates and reactivates it on every config change. During deactivation, the component's service is removed from the registry. If other components hold static references to it, they too are deactivated — cascading outage. Always declare `@Modified` on any component that handles live configuration.

**Key implementation note for extensions**: DS component descriptors (XML files in `OSGI-INF/`) are generated by Bnd at build time from `@Component`, `@Reference`, and `@Activate` annotations. Extensions that build without Bnd must hand-author these XML files. Errors in component descriptors (wrong cardinality, wrong service type) produce silent activation failures — the DS SCR log (trace specification `scr=all`) is the diagnostic tool.

### 2.3 BELL — Bridging Java ServiceLoader to OSGi

**What it is**: BELL (Basic Extensions using Liberty Libraries) bridges the Java `ServiceLoader` pattern (`META-INF/services/<interface-name>`) to the OSGi service registry. When a `<bell libraryRef="...">` element is configured, Liberty:
1. Creates a classloader from the referenced `<library>`'s JARs.
2. Reads `META-INF/services/<interface-name>` entries from those JARs.
3. Instantiates each listed implementation class via reflection.
4. Registers each instance as an OSGi service for the declared interface type.

**Why BELL exists**: Product extensions (full OSGi features) require the extension developer to understand OSGi bundle manifests, DS descriptors, and Liberty feature manifests. BELL lowers the barrier: a developer who already has a standard Java library implementing a Liberty SPI (TAI, `UserRegistry`, `PasswordDecryptionKey`) can register it with zero OSGi knowledge. The `Bell.java` component handles all OSGi registration transparently.

**BELL limitations** (important to know):
- BELL-registered services are plain Java objects — they are **not** DS components. They receive no `@Activate` injection, no `@Modified` callbacks, no `@Reference` resolution. If the implementation needs Liberty services, it must use `ComponentContext.locateService()` or `BundleContext.getServiceReference()`.
- BELL libraries have no declared feature dependencies. If a BELL TAI needs the `<ltpa>` element to exist, it must check this at runtime, not declare it as a prerequisite.
- BELL services are registered in the *application classloader region*, not the kernel region. Packages from the BELL library are only visible within that classloader hierarchy.

**Key entry point**: `com.ibm.ws.classloading.bells/src/com/ibm/ws/classloading/bells/internal/Bell.java` — DS component activated by `<bell>` config; see `activate()` for the `META-INF/services` scan and `BundleContext.registerService()` calls.

### 2.4 Metatype — Schema-Driven Config Injection

**What it is**: Every configurable Liberty bundle ships `resources/OSGI-INF/metatype/metatype.xml`. This file is the schema for the bundle's configuration elements. Config Admin reads metatype metadata from all installed bundles and merges the declared defaults with whatever attributes appear in `server.xml`. The DS component receives a fully-populated `Map<String,Object>` — not a sparse map of only the values in `server.xml`.

**IBM metatype extensions** add richness beyond the baseline OSGi spec:
- `ibm:alias` — controls the XML element name in `server.xml` (e.g., `ibm:alias="dataSource"` → `<dataSource>`); without this, the full PID string is used as the element name.
- `ibm:type="duration"` — the attribute value is parsed as a time duration (`500ms`, `5s`, `2m`) and delivered to the component as a `Long` in milliseconds.
- `ibm:type="location"` — the attribute value is resolved relative to `${server.config.dir}` and validated as a file path.
- `ibm:type="pid"` with `ibm:reference` — the attribute is a reference to another config element by `id`; Config Admin resolves the reference and injects the referenced element's config into the parent component's map.
- `ibm:type="password"` — the attribute value may be an encoded password (`{xor}...`, `{aes}...`); Config Admin decodes it before delivery. Raw passwords in `server.xml` are replaced with their encoded form on first write.
- `cardinality > 1` — the attribute accepts multiple values; delivered as a `String[]`.

**`ibm:flat` for nested config**: Some config patterns nest a child element whose attributes should appear "flat" in the parent's config map. `ibm:flat="true"` on a child AD reference causes Config Admin to merge the child's attributes directly into the parent's map, avoiding a nested map lookup.

**Key entry point**: `com.ibm.ws.config/src/com/ibm/ws/config/xml/internal/MetaTypeRegistry.java` — merges metatype defaults with server.xml values before delivery to DS components.

### 2.5 CDI Portable Extension SPI for User Features

**What it is**: `CDIExtensionMetadata` (in `io.openliberty.cdi.spi`) is the Liberty SPI for registering a CDI portable extension in a user feature or product extension. A DS `@Component` implementing this interface is discovered by Liberty's CDI runtime and its extensions/bean classes are added to every CDI-enabled application that starts on the server.

**Three registration modes** via `CDIExtensionMetadata`:
1. `getExtensions()` — returns a list of CDI `Extension` implementation classes. Liberty's Weld container will load and activate these as standard CDI portable extensions for each application.
2. `getBeanClasses()` — returns classes to be added as CDI beans to every application. These classes act as if they were discovered via bean discovery; they can inject application beans and be injected into application beans.
3. `getBeanDefiningAnnotationClasses()` — returns annotation types that Liberty should treat as bean-defining annotations (i.e., annotations that trigger CDI bean discovery for annotated classes), even if the application uses `bean-discovery-mode="annotated"`.

**Why this SPI exists**: A CDI `Extension` registered via `META-INF/services` inside an application JAR is application-scoped — it only affects that application. A Liberty product extension needs server-scoped CDI integration: inject into all applications, or add behaviour that spans applications. `CDIExtensionMetadata` registered as a DS component achieves server-scoped CDI integration without requiring any `META-INF/services` file in the application archive.

**Key entry point**: `com.ibm.ws.cdi.interfaces/src/io/openliberty/cdi/spi/CDIExtensionMetadata.java` — SPI interface; DS components implementing this are discovered by `com.ibm.ws.cdi.internal` when any CDI-enabled application starts.
